...
class TestClient:
    ...
    def test_mkd(self, dirname):
        ...
        return self.check_server_output(
            expected_server_return
        ) and self.check_client_output(expected_client_return)
    ...
    def test_cwd(self, dirname, pathname):
        ...
        if pathname == "/":
            ...
            expected_server_return = (
                f"[anonymous] CWD {self.server_root_dir}{pathname} 250"
            )
    ...
    def test_retr(self, filename, pathname, filesize=10000):
        ...
        if not (
            (
                client_output_list[0].startswith("150")
                or client_output_list[0].startswith("125")
            )
            and client_output_list[-1].startswith("226")
        ):
            print("Bad response for RETR")
            os.remove(filename)
            os.remove(pathname + "/" + filename)
    ...
    def test_login(self):
        ...
        expected_client_return = "331"
        if not self.check_client_output(expected_client_return):
            return False
        ...
        return self.check_client_output(
            expectecd_client_return
        ) and self.check_server_output(expected_server_return)

    def test_part(self):
        directory_name = "test_" + "".join(
        ...
        credit += (
            1
            if self.test_cwd(dirname=directory_name, pathname="/" + directory_name)
            else 0
        )
        ...
        credit += (
            4
            if self.test_retr(
                "test_retr.data", self.server_root_dir + "/" + directory_name
            )
            else 0
        )
        ...
        credit += (
            4
            if self.test_retr(
                "test_retr_2.data", self.server_root_dir + "/" + directory_name
            )
            else 0
        )
    ...
    def test_public(self):
        ...
        self.reading_thread = threading.Thread(
            target=read_client_output, args=(self.client, self.client_output_queue)
        )
